  ...
  namespace Postprocess
  {
    ...
    class ExtentComposition : public Interface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      ...
      private:
         ...
         * overriding plate
         */
        unsigned int composition_short; 
        /**
    ...
    //   MaterialModel::MaterialModelOutputs<dim> out(quadrature.size(), this->n_compositional_fields());
      ...
      //std::cout<<"name composition selected :"<<this->introspection().name_for_compositional_index(composition_short)<<std::endl;  
      ...
      for (const auto &cell : this->get_dof_handler().active_cell_iterators())
      {
        if (cell->is_locally_owned())
        {
          ...
          for (unsigned int face_no = 0; face_no < GeometryInfo<dim>::faces_per_cell; ++face_no)
          {
                ...
                fe_face_values[this->introspection().extractors.compositional_fields[composition_short]].get_function_values (this->get_solution(),
                ...
                for (unsigned int corner = 0; corner < face_corners.size(); ++corner)
                  ...
                  //   {     
                      if (compositional_values[corner] >= 0.90)
                        {
      ...
      const double max_x = std::abs(Utilities::MPI::min(local_max_x, this->get_mpi_communicator()));
      
      double extent_composition = max_x-min_x;
      //std::cout<<"abs depth :"<<min_depth<<" ending depth :"<<ending_depth<<std::endl;    

      // statistics.add_value ("Maximum depth value for composition " + this->introspection().name_for_compositional_index(composition_short),
      //                           local_min_height);
...
//       }
      ...
      statistics.add_value ("Extent (m)",
                            extent_composition);
      const char *columns[] = { "Extent (m)"};
      ...
      return std::pair<std::string, std::string> ("Extent (m)",
                                                  output_stats.str());
    ...
    ExtentComposition<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          composition_short = prm.get_integer("Composition to track for extent");
