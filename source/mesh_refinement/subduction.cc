  ...
  namespace MeshRefinement
  {
      ...
      public SimulatorAccess<dim>
    {
      ...
      private:
         ...
         double interface_two_upper_depth;
         double interface_two_lower_depth;
         double interface_one_lower_depth;  
         double X_two_zone_refined;
         double X_one_zone_refined;
         double Y_one_zone_refined;
         double Y_two_zone_refined;
         double Z_one_zone_refined;
        ...
        std::vector<unsigned int> compo_six_refinement;
        std::vector<unsigned int> compo_seven_refinement;
        std::vector<unsigned int> compo_eight_refinement;
        std::vector<unsigned int> compo_nine_refinement;       
       
        std::vector<unsigned int> interface_refinement;
        std::vector<unsigned int> interface_two_refinement;        
...
//         unsigned int border_refinement_level;
          ...
          if (cell->is_locally_owned())
            {
                ...
                bool interface_present_two = false;
                bool compo_six_present = false;
                bool compo_seven_present = false;
                bool compo_eight_present = false;                
                bool compo_nine_present =false;
              ...
              fe_values.reinit(cell);                     
                    ...
                    for (unsigned int p=0; p<quadrature.size(); ++p)
                      {
                           if(vertex(0)>X_one_zone_refined && vertex(0)<X_two_zone_refined && vertex(1)>Y_one_zone_refined && vertex(1)<Y_two_zone_refined && vertex(2) > Z_zone_refined)
                            {
                         if (prelim_composition_values[interface_refinement[0]][p] >= 0.60)
                          {
                              if(interface_two_lower_depth <=vertex(2) && vertex(2)<=interface_two_upper_depth){  

                              interface_present_two = true;

                              break;

                              }else if (interface_one_lower_depth <=vertex(2) && vertex(2)<interface_two_lower_depth){  
                        ...
                        if (prelim_composition_values[compo_four_refinement[0]][p] > 0.01)
                          ...
                          if (prelim_composition_values[compo_six_refinement[0]][p] > 0.01)
                          {
                                  
                            compo_six_present = true;
                            
                          }                                
                          if (prelim_composition_values[compo_seven_refinement[0]][p] > 0.01)
                          {
                                     
                            compo_seven_present = true;
                            
                          }                             
                          if (prelim_composition_values[compo_eight_refinement[0]][p] > 0.01)
                          {
                                  
                            compo_eight_present = true;
                            
                          }                                
                           if (prelim_composition_values[compo_nine_refinement[0]][p] > 0.01)
                          {
                                         
                            compo_nine_present = true;
               ...
               //Only continue if at least one is true
                    ...
                    else if (compo_six_present)
                      {
                        refinement_level = compo_six_refinement[1];
                      }                                              
                    else if (compo_seven_present)
                      {
                        refinement_level = compo_seven_refinement[1];

                      }
                    else if (compo_eight_present)
                      {
                        refinement_level = compo_eight_refinement[1];
                      }                      
                    else if (interface_present)
                      {
                        refinement_level = interface_refinement[1];
                      }    
                    else if (interface_present_two)
                      {
                        refinement_level = interface_two_refinement[1];
                      }                                
                    else if (compo_nine_present)
                      {
                        refinement_level = compo_nine_refinement[0];
                      }                                                                                                                                                                       
                    else
                      {
                        refinement_level = compo_nine_refinement[0];
      ...
      prm.enter_subsection("Mesh refinement");
      {
        prm.enter_subsection("Subduction");
        {
          ...
          prm.declare_entry("Compo 5 refinement","",
                            ...
                            "The compositional field number of the crust, its minimum refinement level and "
                            "its maximum refinement level."); 
          prm.declare_entry("Compo 6 refinement","",
                            Patterns::List (Patterns::Integer(0)),
                            "The compositional field number of the crust, its minimum refinement level and "
                            "its maximum refinement level."); 
          prm.declare_entry("Compo 7 refinement","",
                            Patterns::List (Patterns::Integer(0)),
                            "The compositional field number of the crust, its minimum refinement level and "
                            "its maximum refinement level."); 
          prm.declare_entry("Compo 8 refinement","",
                            Patterns::List (Patterns::Integer(0)),
                            "The compositional field number of the crust, its minimum refinement level and "
                            "its maximum refinement level."); 
          prm.declare_entry("Compo 9 refinement","",
                            Patterns::List (Patterns::Integer(0)),
                            "The compositional field number of the crust, its minimum refinement level and "
                            "its maximum refinement level.");              
         ...
         prm.declare_entry ("Refine interface zone 2 if under", "390000", Patterns::Double(0),
                             "the strain rate at which the mesh should start to be refined. Units: $1 / s$");
         prm.declare_entry ("Refine interface zone 2 if on top of", "350000", Patterns::Double(0),
                             "the strain rate at which the mesh should start to be refined. Units: $1 / s$");
         prm.declare_entry ("Refine interface zone 1 if on top of", "280000", Patterns::Double(0),
                             "the strain rate at which the mesh should start to be refined. Units: $1 / s$"); 
         ...
         prm.declare_entry ("Refine domain if y superior at", "350000", Patterns::Double(0),
                             "the strain rate at which the mesh should start to be refined. Units: $1 / s$");
         prm.declare_entry ("Refine domain if on top of", "300000", Patterns::Double(0),
                             "the strain rate at which the mesh should start to be refined. Units: $1 / s$");         
    ...
    Subduction<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        ...
        interface_two_upper_depth = prm.get_double("Refine interface zone 2 if under"); 
        interface_two_lower_depth = prm.get_double("Refine interface zone 2 if on top of"); 
        interface_one_lower_depth = prm.get_double("Refine interface zone 1 if on top of");        
        ...
        Z_one_zone_refined = prm.get_double("Refine domain if on top of");
          ...
          compo_five_refinement = std::vector<unsigned int> (compo_five.begin(),compo_five.end());
          ...
          const std::vector<int> compo_six
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Compo 6 refinement")));

          compo_six_refinement = std::vector<unsigned int> (compo_six.begin(),compo_six.end());

          AssertThrow (compo_six_refinement.size() == 2,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (compo_six_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (compo_six_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the crust cannot be "
                                   "smaller than the minimum level of the whole model. "));   

          const std::vector<int> compo_seven
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Compo 7 refinement")));

          compo_seven_refinement = std::vector<unsigned int> (compo_seven.begin(),compo_seven.end());

          AssertThrow (compo_seven_refinement.size() == 2,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (compo_seven_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (compo_seven_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the crust cannot be "
                                   "smaller than the minimum level of the whole model. "));      
          
          const std::vector<int> compo_eight
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Compo 8 refinement")));

          compo_eight_refinement = std::vector<unsigned int> (compo_eight.begin(),compo_eight.end());

          AssertThrow (compo_eight_refinement.size() == 2,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (compo_eight_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (compo_eight_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the crust cannot be "
                                   "smaller than the minimum level of the whole model. ")); 
          
           const std::vector<int> compo_nine
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Compo 9 refinement")));

          compo_nine_refinement = std::vector<unsigned int> (compo_nine.begin(),compo_nine.end());

          AssertThrow (compo_nine_refinement.size() == 2,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (compo_nine_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (compo_nine_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the crust cannot be "
                                   "smaller than the minimum level of the whole model. "));         
          ...
          const std::vector<int> interface
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Interface 1 refinement")));
          ...
          const std::vector<int> interface_two
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Interface 2 refinement")));

          interface_two_refinement = std::vector<unsigned int> (interface_two.begin(),interface_two.end());

          AssertThrow (interface_two_refinement.size() == 2,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (interface_two_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (interface_two_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the crust cannot be "
                                   "smaller than the minimum level of the whole model. "));
