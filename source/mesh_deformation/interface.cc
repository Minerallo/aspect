...
namespace aspect
{
  ...
  namespace MeshDeformation
  {
    ...
    MeshDeformationHandler<dim>::update_current_surface_heights()
    {
      ...
      // The query points lie immediately below the surface and many of them
      // are located on edges or vertices shared by several cells. The default
      // RemotePointEvaluation settings retain every matching cell and use one
      // coarse bounding box per MPI rank. On large, strongly distributed
      // spherical meshes this produces a very large all-to-all candidate set
      // and can make initialization take many hours.
      //
      // Mesh displacement is continuous, so all cells adjacent to a shared
      // point provide the same value. Select one of them and use one additional
      // level in the bounding-box R-tree to keep the communication sparse.
      typename Utilities::MPI::RemotePointEvaluation<dim>::AdditionalData
      point_cache_settings;
      point_cache_settings.enforce_unique_mapping = true;
      point_cache_settings.rtree_level = 1;
      Utilities::MPI::RemotePointEvaluation<dim> point_cache(point_cache_settings);
