...
//                   ExcMessage("Fastscape can only be run with a box model"));
      ...
      if (restart)
        ...
        if (use_boxlitho)
        {
            ...
            grid_extent[0].second = x_box_extent;
            grid_extent[1].second = y_box_extent;
            grid_extent[2].second = z_box_extent;
            x_repetitions = x_box_repetitions;
            y_repetitions = y_box_repetitions;  
        }else
        {
    ...
    void FastScape<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection ("Mesh deformation");
      {
        prm.enter_subsection ("Fastscape");
        {
          ...
          prm.declare_entry ("Use box with lithosphere", "false",
                             Patterns::Bool (),
                             "Flag to use two merged box geometry model.");          
          
          prm.enter_subsection("Box with lithosphere");
          {
          prm.declare_entry("X box extent", "2208000",
                            Patterns::Double(),
                            "Set a X extent in meters.");  
          prm.declare_entry("Y box extent", "100000",
                            Patterns::Double(),
                            "Y extent when used with 2D"); 
          prm.declare_entry("Z box extent", "100000",
                            Patterns::Double(),
                            "Z extent when used with 2D");           
          prm.declare_entry("X box repetitions", "69",
                            Patterns::Double(),
                            "Set a X repetitions.");
          prm.declare_entry("Y box repetitions", "35",
                            Patterns::Double(),
                            "Set a Y repetitions.");
          }    
    ...
    void FastScape<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection ("Mesh deformation");
      {
        prm.enter_subsection("Fastscape");
        {
          ...
          prm.enter_subsection("Box with lithosphere");
          {
            x_box_extent = prm.get_double("X box extent");
            y_box_extent = prm.get_double("Y box extent");
            z_box_extent = prm.get_double("Z box extent");            
            x_box_repetitions = prm.get_double("X box repetitions");
            y_box_repetitions = prm.get_double("Y box repetitions");
          }
          prm.leave_subsection();
