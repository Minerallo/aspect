...
namespace aspect
{
  namespace Postprocess
  {
     ...
     */
    template <int dim>
    class SinkingDepth : public Interface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      public:
        /**
    ...
    SinkingDepth<dim>::execute (TableHandler &statistics)
    {
    ...
    //   MaterialModel::MaterialModelOutputs<dim> out(quadrature.size(), this->n_compositional_fields());
      ...
      statistics.add_value ("Sinking_vel (m/yr)",
                            min_depth/a_dt);
      const char *columns[] = { "Sinking_vel (m/yr)"};
      ...
      output_stats << min_depth/a_dt << " m/yr";
      ...
      if (!write_to_file || ((this->get_time() < last_output_time + output_interval)
        ...
        return std::pair<std::string, std::string> (" Sinking_vel of the selected composition (m/yr)",
                                                    output_stats.str());


      return std::pair<std::string, std::string> ("Sinking_vel (m/yr)",
                                                  output_stats.str());
    ...
    template <int dim>
    void SinkingDepth<dim>::declare_parameters(ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    SinkingDepth<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Box with lithosphere boundary indicators");
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(SinkingDepth,
                                  "Sinking velocity",
